...
impl ProtonCalendarMock for MockServer {
    #[function_name::named]
    async fn mock_get_calendar_bootstrap(&self, event_id: &str, response: CalendarBootstrap) {
        Mock::given(method("GET"))
            .and(path(format!("/api/calendar/v1/{event_id}/bootstrap")))
    ...
    async fn mock_get_calendar_event(
        ...
        let mock = Mock::given(method("GET"))
            ...
            .and(query_param("UID", event_id));

        let mock = match recur_id {
