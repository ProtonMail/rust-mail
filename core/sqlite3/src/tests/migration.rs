...
async fn test_migration() {
    ...
    let _version = migrator
        .migrate(&mut tether, TEST_TABLE_NAME, &[M1 {}])
        .await
        .expect("Failed to run migration");

    // second version
    let _version = migrator
        .migrate(&mut tether, TEST_TABLE_NAME, &[M2 {}])
        .await
        .expect("Failed to run migration");
}
...
async fn test_migration_with_different_table_ids() {
    ...
    // first version
    let _version = migrator
        .migrate(&mut tether, TEST_TABLE_NAME_1, &[M1 {}])
        .await
        .expect("Failed to run migration");

    // second version
    let _version = migrator
        .migrate(&mut tether, TEST_TABLE_NAME_2, &[M2 {}])
        .await
        .expect("Failed to run migration");
}
