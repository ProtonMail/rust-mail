...
/// To specify the URI for instant messaging and presence protocol communications with the object
/// the vCard represents.
#[derive(Clone)]
pub struct Impp {
    /// Value (ex: xmpp:alice@example.com)
    pub value: Uri,
...
impl Impp {
    /// Create a nes IMPP property without any parameter or group
    #[must_use]
    ...
    /// Try to create a new IMPP property without any parameter or group
    ///
    /// # Errors
    ...
    pub fn new_validated(value: &str) -> VCardResult<Self> {
        Ok(Self {
            value: Uri::new_validated(value)
                .map_err(VCardError::from_value_error(PropertyKind::Impp))?,
            value_type: None,
            pid: None,
            preference: None,
            r#type: HashSet::new(),
...
impl TryFrom<&IcalProperty> for Impp {
    type Error = VCardError;

    fn try_from(property: &IcalProperty) -> VCardResult<Self> {
        let Some(value) = &property.value else {
            return Err(VCardError::MissingValue(PropertyKind::Impp));
        };
                    ...
                    ParameterType::Value => {
                        result.value_type = Some(
                            ValueType::try_from(values.as_slice())
                                .map_err(VCardError::from_parameter_error(PropertyKind::Impp))?,
                        );
                    }
                    ParameterType::Pid => {
                        result.pid = Some(
...
impl VcardProperty for Impp {
    fn get_preference(&self) -> Option<Preference> {
        self.preference
    }
}

/// Validate that the given `property` respect the format for a `IMPP` property
///
/// # Errors
///   * if property value is not a valid uri
///   * if any of the parameters is invalid
pub fn validate_impp(property: &IcalProperty) -> VcardValidationResult<()> {
