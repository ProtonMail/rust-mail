...
impl Member {
    ...
    /// Try to create a new MEMBER property without any parameter or group
    ///
    /// # Errors
    ...
    pub fn new_validated(value: &str) -> VCardResult<Self> {
        Ok(Self {
            value: Uri::new_validated(value)
                .map_err(VCardError::from_value_error(PropertyKind::Member))?,
            value_type: None,
            pid: None,
            preference: None,
            media_type: None,
...
/// Validate that the given `property` respect the format for a `MEMBER` property
///
/// # Errors
///   * if property value is not a valid uri value
///   * if any of the parameters is not valid
pub fn validate_member(property: &IcalProperty) -> VcardValidationResult<()> {
