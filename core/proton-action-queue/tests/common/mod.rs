...
use proton_action_queue::{
    ActionFactory, ActionQueue, ActionStore, SessionProvider,
...
impl TestCtx {
    pub async fn new() -> TestCtx {
        ...
        ActionStore::init_tables(&stash).await.expect("failed to init store tables");

        let _ = TestLocalSource::new_with_init(stash.clone()).await.expect("failed to crease local source");
        TestCtx {
            stash,
            _file: tmp_file,
...
async fn test_local_source() {
    ...
    let transaction = test_ctx.stash().transaction().await.expect("failed to start transaction");
    let mut tx = TestLocalSourceTransaction::new(transaction.clone());
            let folder_id1 = tx.create_folder("foo").await.expect("failed to create folder");
            let folder_id2 = tx.create_folder("bar").await.expect("failed to create folder");
            let folder_id3 = tx.create_folder("xxx").await.expect("failed to create folder");
            ...
            let message_id1 = tx.create_message(false).await.expect("failed to create message");
            let message_id2 = tx.create_message(false).await.expect("failed to create message");
            let message_id3 = tx.create_message(true).await.expect("failed to create message");

            tx.move_message_to_folder(&[message_id2], folder_id1).await
                .expect("failed to move");
            tx.move_message_to_folder(&[message_id3], folder_id2).await
                .expect("failed to move");
            {
                let message = tx
                    .get_message(message_id1).await
            ...
            {
                let messages = tx
                    .get_messages(&[message_id1, message_id2, message_id3]).await
            ...
            {
                assert!(tx
                    .get_message(MessageId(25)).await
                    .expect("failed to get message")
                    .is_none());
            }
            {
                let name = tx
                    .get_folder_name(folder_id1).await
            ...
            {
                let name = tx
                    .get_folder_name(folder_id2).await
                    .expect("failed to get folder")
                    .expect("folder should be found");
                assert_eq!(name, "bar");
            }
            {
                let name = tx
                    .get_folder_name(folder_id3).await
                    .expect("failed to get folder")
                    .expect("folder should be found");
                assert_eq!(name, "xxx");
            }
            ...
            {
                tx.rename_folder(folder_id3, "renamed").await
            ...
            //delete folder
            {
                tx.delete_folder(folder_id3).await
                    .expect("failed to delete folder");
                let message = tx
                    .get_message(message_id3).await
                    .expect("failed to get message")
                    .expect("message should be found");
            ...
            // add labels
            {
                tx.add_message_to_label(&[message_id3], label_id1).await
                    .expect("failed to add to folder");
                tx.add_message_to_label(&[message_id3], label_id2).await
                    .expect("failed to add to folder");
                let message = tx
                    .get_message(message_id3).await
                    .expect("failed to get message")
            ...
            // remove folder
            {
                tx.remove_message_from_label(&[message_id2], label_id2).await
                    .expect("failed to remove from folder");
                tx.remove_message_from_label(&[message_id2], label_id1).await
                    .expect("failed to remove from folder");
                let message = tx
                    .get_message(message_id2).await
                    .expect("failed to get message")
            ...
            // Read unread
            {
                tx.mark_messages_read(true, &[message_id1]).await
            ...
            // Delete messages
            {
                tx.delete_message(&[message_id1, message_id2]).await
                    .expect("failed to delete messages");
            }

    transaction.commit().await.expect("Failed to execute transaction");
}
