...
use proton_mail_html_transformer::transforms::styles::{
    dark_mode_for_plaintext, BrowserCapabilities, IncludeFullStaticCss
};
...
pub fn inject_dark_mode(
    ...
    if mime_type == MimeType::TextPlain {
        return DarkModeInjection {
            head: dark_mode_for_plaintext(color_mode, capabilities, IncludeFullStaticCss::Yes).to_owned(),
            body: body.to_owned(),
        };
    }
