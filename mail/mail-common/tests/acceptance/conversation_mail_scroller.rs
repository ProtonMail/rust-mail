...
async fn test_conversation_mail_scroller_reads_correct_items_within_visible_range_for_cached_scroll_data()
 {
    ...
    let mut test_scroller =
        TestScroller::conversations(&user_ctx, local_label_id, read, include, page_size)
            .await
            .unwrap();
...
async fn test_conversation_mail_scroller_reads_non_empty_folder_for_the_first_time_and_api_data_is_equal_to_the_cache()
 {
    ...
    let page_size = 10;
    let mut test_scroller =
        TestScroller::conversations_instant(&user_ctx, local_label_id, read, include, page_size)
            .await
            .unwrap();
...
async fn test_conversation_mail_scroller_handles_create_or_get_local_missing_labels() {
    ...
    let mut inbox_scroller =
        TestScroller::conversations(&user_ctx, inbox_local_label_id, read, include, page_size)
            .await
            .unwrap();
