...
### Added
  ...
  - `account_details` function in `MailUserSession` and `details` function in `StoredAccount` (it contains account name, email and avatar information that needs to be displayed to the user).
...
### Removed
  ...
  - `avatar_information`, `display_name`, `name_or_addr`, `primary_addr`, `username` functions from `StoredAccount` (replaced with `details` function).
