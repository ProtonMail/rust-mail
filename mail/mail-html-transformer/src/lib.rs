...
use transforms::{Stylesheet, keep_spaces_and_escape_gt_and_lt};
...
impl Transformer {
    ...
    #[tracing::instrument(level = tracing::Level::DEBUG, skip_all)]
    pub fn inject_style(&mut self, style: Stylesheet) {
        transforms::inject_style(self.document.clone(), style);
    }
