...
use crate::test::server::error::{cerr, serr, ServerRes};
...
impl Backend {
    ...
    pub async fn new_user(&self, name: &str, pass: &str) -> ServerRes<UserId> {
        ...
        let salt = crypto::new_srp_salt();
        let addr = format!("{name}@foo.com");

        // Create the user's SRP verifier.
        let Ok((version, pass, verifier)) = crypto::new_srp_verifier(pass, &salt) else {
            return Err(serr!("failed to generate verifier"));
        };

        // Generate the user's PGP key with the (now hashed) password.
        let Ok(key) = crypto::new_raw_pgp_key(name, &addr, &pass) else {
            return Err(serr!("failed to generate PGP key"));
        };

        // Armor the key.
        let Ok(arm) = crypto::to_arm_pgp_key(&key) else {
            return Err(serr!("failed to armor PGP key"));
        };
        ...
        let key = Key::new(arm);
        lock.keys.insert(key_id, key);
        info!(%key_id, "created new key");

        let user_id = UserId::new();
        let user = User::new(name, addr, salt, verifier, [key_id], version);
    ...
    pub async fn new_srp_session(
        ...
        let Ok((challenge_id, challenge, modulus)) = crypto::new_srp_challenge(&verifier) else {
            return Err(serr!("failed to generate challenge"));
        };

        let mut lock = self.inner.lock().await;

        let srp_id = SrpId::new();
        let srp = Srp::new(challenge_id, user_id);
        lock.srp.insert(srp_id, srp);
        info!(%srp_id, "created new SRP session");

        Ok((srp_id, challenge, modulus))
    }
    ...
    pub async fn verify_srp_proof(
        ...
        let Some(srp) = self.inner.lock().await.srp.remove(&srp_id) else {
        ...
        let Ok(proof) = crypto::verify_srp_proof(srp.challenge_id, ephemeral, proof) else {
...
impl User {
    fn new(
        ...
        verifier: impl Into<Vec<u8>>,
        key: impl IntoIterator<Item = KeyId>,
        srp: u8,
    ) -> Self {
        Self {
            name: name.into(),
...
#[derive(Debug, Clone)]
struct Srp {
    /// The challenge ID (to complete the challenge).
    challenge_id: u64,
...
impl Srp {
    fn new(challenge_id: u64, user_id: UserId) -> Self {
        Self {
            challenge_id,
            user_id,
        }
