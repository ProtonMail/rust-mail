//! This example demonstrates the basic login flow for a muon client with an info provider
//! The info provider is used by the muon client to fetch the fingerprint and attach it to the login call
...
// An example implementation of InfoProvider used by the muon Client to fetch the fingerprint.
...
async fn main() -> Result<()> {
    ...
    // The info_provider is used to fetch the fingerprint from the apps that integrate muon.
    // This fingerprint is attached to the login call to help prevent fraud.
