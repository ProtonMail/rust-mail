...
async fn test_auth_with_fingerprint_provider() -> Result<()> {
    ...
    // We do this here to make sure the fingerprint we provide with the provider is used
    // The result of using a valid fingerprint is indistinguishable from using no fingerprint
    // So we use the error as a way to make sure the fingerprint we specify is sent.
...
async fn test_auth_with_both_fingerprint_apis() -> Result<()> {
    // If both apis are used, we use the fingerprint passed to LoginExtraInfo for the login call
    // We pass in an invalid fingerprint to LoginExtraInfo and a valid one for the provider
    // If the call fails we can be sure the LoginExtraInfo fingerprint was used
    let extra_info = LoginExtraInfo::builder()
