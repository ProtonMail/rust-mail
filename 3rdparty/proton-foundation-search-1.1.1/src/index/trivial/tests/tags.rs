...
fn should_create_index() {
    ...
    6DC04A12BB655F8B8B33D84C92437A61: {
      "0": {
        "0": {
          "0": [
            0
...
fn should_move_entry() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            (EntryIndex(2), AttributeIndex(1), (vec![Some("129".into())])),
...
fn should_find_matching() {
    ...
    entries.walk(|entry_idx, attr_idx, value| {
        for value in value {
            let Some(EntryValue::Tag(value)) = value else {
                continue;
            };
            ...
            assert_eq!(found, vec![entry_idx.0], "searching for {value:?}");
        }
    });
}
...
fn should_filter_by_prefix() {
    ...
    let search = sut
        .search(
            ...
            Func::Equals,
            &TermValue::text("2").wildcard(),
            &Default::default(),
        )
        .expect("search");
    assert_eq!(found(search, &cache), vec![0, 1]);
}
