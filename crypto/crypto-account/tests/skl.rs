...
use proton_crypto_account::domain::{
    APIPublicAddressKeys, APIPublicKey, APIPublicKeySource, KeyFlag, PublicAddressKeys,
    SKLSignature, SignedKeyList,
};

fn get_test_keys<T: PGPProviderSync>(provider: &T) -> PublicAddressKeys<T::PublicKey> {
    let api_address_keys = APIPublicAddressKeys(vec![
        APIPublicKey{
    ...
    api_address_keys.import(provider)
}
